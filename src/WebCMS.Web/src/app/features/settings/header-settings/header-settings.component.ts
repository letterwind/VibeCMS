...
export class HeaderSettingsComponent implements OnInit {
  ...
  loadSettings(): void {
    this.loading = true;
    this.settingsService.getHeaderSettings().subscribe({
      next: (result) => {
      ...
      error: (err) => {
        ...
        this.showMessage(this.languageService.getTranslation('headerSettings.loadFailed'), true);
        this.loading = false;
      }
    });
  }
  ...
  save(): void {
    ...
    this.settingsService.updateHeaderSettings(request).subscribe({
      next: (result) => {
        this.settings = result;
        this.editorContent = result.htmlContent || '';
        this.showMessage(this.languageService.getTranslation('headerSettings.saveSuccess'), false);
        this.saving = false;
      },
      error: (err) => {
        console.error('Failed to save header settings:', err);
        this.showMessage(this.languageService.getTranslation('headerSettings.saveFailed'), true);
        this.saving = false;
      }
    });
  }
